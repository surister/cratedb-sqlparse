...
Query properties can be read with `statement.metadata.with_properties`.

Properties are only detected in statements with `WITH`.
...
Interpolated properties are properties without a defined value, they 
...
For values to be picked up they need to start with a dollar `'$'` and be numeric, e.g. `'$1'`, `'$123'` -
